...
def fetch_recipes():
    """
    Fetch recipes from the `filtered_recipes_clustered` table.
    """
    conn = create_connection()
    try:
        query = "SELECT id, name, cluster, description, calories, time, vegetarian FROM `User-restrictions`.`filtered_recipes_clustered`;"
...
if __name__ == "__main__":
    # Fetch recipes from the SQL database
    recipes_df = fetch_recipes()
    recipes = recipes_df[["name", "id", "cluster", "description", "calories", "time", "vegetarian"]].to_dict(orient="records")
